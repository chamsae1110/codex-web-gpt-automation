...
def test_bootstrap_script_uses_live_devspace_allowed_roots_contract() -> None:
    ...
    assert "$Invocation.ConfigSha256" in text
    assert "$Recovery.service_started" in text
    assert "$Service.supervisor_pid" in text
...
def test_watch_mode_reloads_allowed_roots_each_cycle(tmp_path: Path) -> None:
    ...
    helper.write_text(
        ...
        "print(json.dumps({'ok': True, 'service_started': False}))\n",
        encoding="utf-8",
    )
    bootstrap = tmp_path / "bootstrap.json"
    ...
    completed = subprocess.run([
        ...
        "-File", str(SCRIPT), "-CodexHome", str(codex_home),
        "-ConfigPath", str(bootstrap), "-DevSpaceConfigPath", str(devspace),
    ...
    calls = [json.loads(line) for line in capture.read_text(encoding="utf-8").splitlines()]
    assert len(calls) == 2
    assert str(first.resolve()) in calls[0]
    assert str(second.resolve()) not in calls[0]
    assert str(first.resolve()) in calls[1]
    assert str(second.resolve()) in calls[1]
