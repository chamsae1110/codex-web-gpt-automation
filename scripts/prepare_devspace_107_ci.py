#!/usr/bin/env python3
"""Prepare the exact published DevSpace 1.0.7 package for CI tests."""
...
DEVSPACE_INTEGRITY = "sha512-kP+Wk52qiMRwdqAP+nV4OZ4HU8feivZQ0k6u4ZUkvqxu8j0Rp/AU8H0K4T43G+zmu9WJKlYLTet7vIUeZHU72A=="
MAX_FILES = 10_000
...
def main() -> int:
    ...
    stage = Path(tempfile.mkdtemp(prefix="devspace-107-ci-", dir=runner_temp))
    ...
    with github_env.open("a", encoding="utf-8", newline="\n") as output:
        output.write(f"DEVSPACE_107_PACKAGE_ROOT={package_root}\n")
        output.write(f"DEVSPACE_107_PACKAGE_ARCHIVE={archive}\n")
