...
try {
  ...
  function Get-RecoveryInvocation {
    ...
    return [pscustomobject]@{
      Arguments = $Arguments
      RootCount = $LiveRoots.Count
      ConfigSha256 = Get-TextSha256 $RawConfig
    }
  }
  ...
  while ($true) {
    ...
    for ($Attempt = 1; $Attempt -le 6; $Attempt++) {
      ...
      if ($ExitCode -eq 0) {
        ...
        try {
          $Recovery = $RecoveryText | ConvertFrom-Json -ErrorAction Stop
        } catch {
          $Digest = Get-TextSha256 $RecoveryText
          Write-BootstrapLog "Recovery cycle $Cycle attempt $Attempt returned invalid JSON (sha256=$Digest)."
          $ExitCode = 1
          continue
        }
        if ([bool]$Recovery.service_started) {
          $Service = $Recovery.service
          Write-BootstrapLog ("DevSpace restarted (cycle={0}, attempt={1}, supervisor_pid={2}, child_pid={3}, started_at={4}, state={5}, stdout={6}, stderr={7})." -f $Cycle, $Attempt, $Service.supervisor_pid, $Service.child_pid, $Service.started_at, $Service.state_path, $Service.stdout_path, $Service.stderr_path)
        }
        $Healthy = $true
        break
      }
      $FailureDigest = Get-TextSha256 (($RecoveryOutput -join [Environment]::NewLine).Trim())
      Write-BootstrapLog "Recovery cycle $Cycle attempt $Attempt failed with exit code $ExitCode (output_sha256=$FailureDigest)."
