...
full-access DevSpace for mission-owned mutations, commands/tests, network,
browser/CDP verification, and iterative agentic work. Explicit `pro-attachment`
remains a separate read-only immutable-evidence route; persisted
`pro-devspace-readonly` records retain their exact recovery semantics. Each web stage authors the next
