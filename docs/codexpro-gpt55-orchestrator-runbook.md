...
New orchestrator work uses Oracle plus DevSpace. Explicitly requested new Pro
uses the same exact-root DevSpace route read-only for design, advice, or review;
a regular `GPT-5.6` `extra-high` stage owns mutations and commands. Explicit
`pro-attachment` remains a separate read-only immutable-evidence route.
Persisted legacy `pro-devspace` write records retain their exact authority only during recovery.
