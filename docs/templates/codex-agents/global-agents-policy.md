...
- A same-conversation read-only Pro round may use only the runner's internal `followup` command against a task-bound terminal `pro-devspace-readonly` parent. Raw `--followup`, `--browser-follow-up`, and `session` injection stay forbidden. An archived exact parent may be restored only for that bound round and must be re-archived afterward. Each round must prove the unchanged conversation, archive transition, and append hash-bound reservation/result receipts; an unproven or changed conversation fails closed without a replacement prompt.
