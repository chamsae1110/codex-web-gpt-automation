...
- Every new ChatGPT submission uses Oracle. Regular web work defaults to `GPT-5.6` at the highest supported non-Pro reasoning tier (`extra-high`). Pro is quota-limited and must never be inferred from task difficulty. An explicit user request may either select one Pro run or persist `regular_web_mode: pro` in the host-only `chatgpt-workspace.json`; that durable setting is explicit opt-in for later ordinary, comprehensive, and Web Multi web sessions. Qualified Pro uses `GPT-5.6 Sol` at the Pro effort with the same DevSpace app and mission-scoped read/write/command authority inside the exact root. Deep Research remains its separate non-Pro research mode, and explicit `pro-attachment` remains limited to immutable or external evidence that DevSpace cannot read.
