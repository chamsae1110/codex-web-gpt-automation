...
- Every new ChatGPT submission uses Oracle. Regular web work defaults to `GPT-5.6` at the highest supported non-Pro reasoning tier (`extra-high`). Pro is quota-limited and must never be selected or upgraded automatically: only an explicit user Pro request or durable explicit Pro preference may select `GPT-5.6 Sol` at the Pro effort. Every new qualified Pro run uses the `pro-devspace` full-access DevSpace route and may perform mission-owned file mutations, shell commands and tests, network access, browser or Chrome DevTools/CDP verification, and the complete inspect-plan-execute-test-inspect-adapt-verify agentic loop. Exact-root binding, repository rules, and explicit approval boundaries for destructive, credential, account, deployment, publication, purchase, or other external-state actions remain authoritative. Explicit `pro-attachment` remains a separate read-only immutable/external-evidence route and is never an automatic fallback. Persisted `pro-devspace-readonly` and historical attachment runs retain their exact original authority only during recovery.
...
- A new Pro DevSpace stage has the same exact-root binding and maximum mission-authorized DevSpace capability. It may create, edit, or remove mission-owned files; run commands and tests; use network access; and inspect or control a user-authorized browser through Chrome DevTools/CDP. The installed `.codex/bin/chatgpt_chrome_cdp.mjs` helper exposes loopback target listing, DOM/JavaScript evaluation, and arbitrary CDP method calls. Project rules and explicit irreversible-action boundaries remain authoritative. Persisted read-only Pro runs preserve their original authority during exact recovery. Once the one-time DevSpace qualification is complete, do not re-check app/settings state per run.
- Standalone Pro returns one durable advisory and stops. It does not require a `WEB_MULTI_NEEDED` footer or automatically launch Web Multi; only a separate explicit user request authorizes that extra provider work. An ordinary supported live Pro dispatch performs fail-closed preflight internally and must not be preceded by a duplicate routine dry-run; keep dry-run for changed automation/configuration, unfamiliar manifests, troubleshooting, or an explicit preview request.
- Before the first DevSpace-backed Oracle submission for a new exact project root, verify that it is equal to or contained by a current local DevSpace `allowedRoots` boundary. An explicitly approved parent covers descendant projects, but the worker must still open the mission's exact root; a child, sibling-prefix, or other-drive path does not qualify. Cache that qualification against the config hash and revalidate only when the config changes. This lightweight root guard must not automate or repeatedly inspect ChatGPT app/settings state.
...
- The GPT-5.6 Pro path excludes agbrowse from creation, dispatch, verification, and fallback. Keep agbrowse installed only for separate public-web search/browser QA and exact persisted legacy-run recovery.
