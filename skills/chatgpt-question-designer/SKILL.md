...
4. `Evidence boundary`: list the live DevSpace workspace scope for regular GPT and qualified Pro, web/source constraints, freshness limits, and what cannot be inspected. New qualified Pro uses full-access DevSpace with mission-owned authority. A separately explicit `pro-attachment` request may use a read-only immutable/external-evidence packet as a new route; it is never an automatic fallback from DevSpace. Persisted legacy attachment runs keep their exact recovery contract.
...
- Regular web work defaults to `gpt-5.6` at the highest supported non-Pro reasoning tier. Pro is quota-limited and may be selected only after an explicit user request or durable explicit Pro preference; never infer or auto-upgrade to it. Every new qualified Pro run uses Oracle, `GPT-5.6 Sol` at the Pro effort, and full-access DevSpace at the exact project root. It may read and mutate mission-owned files, run commands and tests, use network access, and perform browser or Chrome DevTools/CDP verification. For implementation it owns the complete inspect, plan, execute, test, inspect-result, adapt, and verify loop. Explicit `pro-attachment` remains a separate read-only immutable/external-evidence route and is never an automatic fallback.
...
- Planning/review: identify the live draft, research, acceptance criteria, local guidance, and known risks by project-relative paths in the mission. New Pro planning/review remains inside full-access DevSpace but follows the action authority declared by that mission; use an explicit attachment packet only for its immutable/external-evidence boundary.
